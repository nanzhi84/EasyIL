"""Standalone evaluation script and utilities for trained policy models (JAX)."""
...
def load_eval_context(
    ...
    module.init_state(
        ...
        weight_decay=train_cfg.train.get("weight_decay", 0.0),
        ema_decay=train_cfg.algo.get("ema_decay", 0.999),
    )
...
def run_eval(
    ...
    *,
    ...
    # Check if module is diffusion-based (has sample_actions with rng_key)
    is_diffusion = hasattr(module, "scheduler")
    ...
    while len(finished_returns) < n_episodes:
        ...
        if need_plan:
            ...
            if is_diffusion:
                ...
                chunks = module.sample_actions(sample_key, obs_batch, use_ema=True)
            else:
                chunks = module.sample_actions(obs_batch, use_ema=True)
...
def main(cfg: DictConfig) -> None:
    ...
    rng_key = jax.random.PRNGKey(seed + 12345)
